...
var api = {
		...
		doc: { //设备备件分类
			spexcal: BASE_WEB + 'doc/备件导入.xlsx' //备件信息导入模板
		}
	};

	var SoaIot = {
		getUrlParam: function(name) { //获取URL连接参数
		...
		getTimemmm: function() {//获取当前日期
			return Date.now();
		},
		removeAaary: function(arr1, arr2) {
			var arr3 = [];
		...
		getCookis: function(cookieName) {//获取cookis
			var strCookie = document.cookie;
			var arrCookie = strCookie.split("; ");
			for (var i = 0; i < arrCookie.length; i++) {
				var arr = arrCookie[i].split("=");
				if (cookieName == arr[0]) {
					return decodeURI(arr[1]).replace(/\"/g, '');
				}
			}
			return "";

		},
		transitionDate: function(Time) {//时间格式化
			if (Time == null)
				return '';

			var dateee = new Date(Time).toJSON();
			return new Date(+new Date(dateee) + 3600 * 1000).toISOString().replace(/T/g, ' ').replace(/\.[\d]{3}Z/, '');
